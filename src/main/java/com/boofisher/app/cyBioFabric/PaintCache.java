...
public class PaintCache {
  ...
  */
  ...
  private void buildALineVert(BioFabricNetwork.LinkInfo link, List<PaintedPath> objCache, FabricColorGenerator colGen, boolean showShadows) {
    ...
    int yStrt = sRow * GRID_SIZE;
    int yEnd = eRow * GRID_SIZE;
    int x = num * GRID_SIZE;
    ...
    if (!link.isDirected()) {
      ...
      int ySrc = link.getStartRow() * GRID_SIZE;
      int yTrg = link.getEndRow() * GRID_SIZE;
  ...
  */
  ...
  private void buildALineHorz(BioFabricNetwork.NodeInfo target, List<PaintedPath> preCache, 
    ...
    if (dzmm != null) {
      ...
      if (bounds.getWidth() <= (GRID_SIZE * diff)) {
        useFont = 0;
        dumpRect = bounds;
      } else {
    ...
    float tnamey = (target.nodeRow - DRAIN_ZONE_ROW_OFFSET_) * GRID_SIZE;
    ...
    float namey = (target.nodeRow * GRID_SIZE) + (float)(scaleHeight / 2.0);
    float namex = (colmm.min * GRID_SIZE) - (float)labelBounds.getWidth() - BB_RADIUS_ - NODE_LABEL_X_SHIM_;      
    ...
    if (dumpRect != null) {
      ...
      if (doRotateName) {
        ...
        tnamex = (dzmm.min * GRID_SIZE) + ((diff * GRID_SIZE) / 2.0F)  - ((int)dumpRect.getWidth() / 2);
    ...
    //
    int yval = (target.nodeRow * GRID_SIZE);
    int xStrt = (colmm.min * GRID_SIZE);
    int xEnd = (colmm.max * GRID_SIZE);
  ...
  */
  ...
  private void buildABackRect(MinMax dzmm, Map<Integer, MinMax> linkExtents, Rectangle2D dumpRect, List<PaintedPath> preCache, Color col) {  
    ...
    int rectLeft = (int)Math.floor((double)(dzmm.min * GRID_SIZE) - BB_RADIUS_ - (STROKE_SIZE / 2.0));
    int topRow = (int)Math.floor((double)(minRow * GRID_SIZE) - BB_RADIUS_ - (STROKE_SIZE / 2.0));
    int rectTop = (dumpRect == null) ? topRow : Math.min((int)dumpRect.getMinY(), topRow);
    int rectRight = (int)Math.ceil((double)(dzmm.max * GRID_SIZE) + BB_RADIUS_ + (STROKE_SIZE / 2.0));
    int rectWidth = rectRight - rectLeft;
    int rectBot = (int)Math.floor((double)(maxRow * GRID_SIZE) + BB_RADIUS_ + (STROKE_SIZE / 2.0));
