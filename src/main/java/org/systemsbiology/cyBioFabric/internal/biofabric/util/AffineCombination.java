...
package org.systemsbiology.cyBioFabric.internal.biofabric.util;
...
public class AffineCombination {
  ...
  */
  ...
  public static Point2D combination(List<Point2D> points, double[] weights, double gridVal) {
  ...
  */
  ...
  public static List<Double> getWeights(List<Point2D> points, Point2D point) {
    ...
    Map<Point2D, ArrayList<Integer>> indexMap = createIndexMap(points);
    // This ditches duplicates:
    ArrayList<Point2D> working = new ArrayList<Point2D>(indexMap.keySet());
    List<Point2D> ordered = orderPoints(working, point);
    if (ordered == null) {
      return (null);
    }
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (getWeights(point, pointSet, toFill)) {
        weights.get(indices[0]).add(new Double(toFill[0]));
        weights.get(indices[1]).add(new Double(toFill[1]));
        weights.get(indices[2]).add(new Double(toFill[2])); 
  ...
  */
  ...
  private static Map<Point2D, ArrayList<Integer>> createIndexMap(List<Point2D> points) {
  ...
  */
  ...
  private static List<Double> distributeWeights(Map<Point2D, ArrayList<Integer>> indexMap, List<Point2D> ordered, 
		  ArrayList<ArrayList<Double>> weights, double sum, int origSize) {
  
    double[] preRetval = new double[origSize];
    int size = ordered.size();
    for (int i = 0; i < size; i++) {
      Point2D point = (Point2D)ordered.get(i);
      ArrayList<Double> weightList = weights.get(i);
  ...
  */
  ...
  private static List<Point2D> orderPoints(List<Point2D> points, Point2D ref) {
    ...
    Iterator<OrderedPoint> sit = sorted.iterator();
    while (sit.hasNext()) {
  ...
  */
  ...
  private static void chooseBasisIndices(List<Point2D> points, int startIndex, int[] indices) {
    //
  ...
  */
  ...
  /*public static void main(String[] argv) {
    ...
    List<Double> weightList = AffineCombination.getWeights(pointList, origPoint);
